"""
@summary: Test's the Lifemapper openModeller configuration
@author: CJ Grady
@version: 2.0
@status: beta
...
from config import OM_MODEL_CMD, OM_PROJECT_CMD
...
def testRunModelJob(env):
   ...
   """
   j = env.requestJob(validTypes=[1], parameters={'users': 'lm2'})
   jr = OMModelRunner(j, env)
   jr.run()
...
def testRunProjectionJob(env):
   ...
   @param env: An environemnt class to test
   """
   j = env.requestJob(validTypes=[2], parameters={'users': 'lm2'})
   jr = OMProjectionRunner(j, env)
   jr.run()
